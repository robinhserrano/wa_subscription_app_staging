      ...
      <font-awesome-icon :icon="['fas', 'link-slash']" :style="{ color: '#800000' }" />
            ...
            <Column field="sales_order_no" header="Sales Order No." style="min-width: 10rem">
                <template #body="{ data }">
                    ...
                    <!-- <p v-if="data.order_line.length > 2">
                    abc
                  </p> -->
            ...
            <Column v-if="route().current('confirmDeliveryFilterSubscription')" field="" header="Require Delivery"
...
const fetchData = async () => {
    try {
        ...
        console.log(currentPage.value)
        const response = router.get('/confirmDeliveryFilterSubscription', {
...
const debouncedFetchData = debounce(async () => {
    try {
        console.log('fetch data page');
        console.log(currentPage.value);
        const response = await router.get('/confirmDeliveryFilterSubscription', {
            page: currentPage.value,
            search: search.value,
            dates: dates.value,
            stateId: selectedStateIds.value,
